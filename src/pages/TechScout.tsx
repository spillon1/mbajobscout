...
import { getTechDefaultSources, TECH_DEFAULT_KEYWORDS } from '@/data/techData';
...
function toTechUrl(url: string): string {
  return url
    .replace(/venture\+capital/gi, 'technology+product+management')
    .replace(/venture%20capital/gi, 'technology%20product%20management')
...
const TechScout = () => {
  ...
  const [selectedCity, setSelectedCity] = usePersistedState<string>('tech-city', 'London');
  const location = getLocationString(selectedCity);
  const [sources, setSources] = useState<JobSource[]>(() => getTechDefaultSources('London'));
  const [keywords, setKeywords] = useState<string[]>(TECH_DEFAULT_KEYWORDS);
  const [jobs, setJobs] = useState<Job[]>([]);
  const [dismissedIds, setDismissedIds] = useState<Set<string>>(new Set());
  const [hasScraped, setHasScraped] = useState(false);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => { loadSavedJobs('tech').then((saved) => { if (saved.length > 0) { setJobs(saved); setHasScraped(true); } setIsLoading(false); }); }, []);
  useEffect(() => { const result = consumeResults('tech'); if (result) { setJobs(result.jobs); setDismissedIds(new Set()); setHasScraped(true); setSources((prev) => prev.map((s) => { const ss = result.sourceStatuses[s.name]; return { ...s, status: ss?.status as any || s.status, statusMessage: ss?.error || (ss?.status === 'connected' ? `Scraped successfully (${ss?.count ?? 0} jobs found)` : undefined), lastJobCount: ss?.count ?? undefined }; })); } }, [scrapeState.isSearching, consumeResults]);

  const [viewMode, setViewMode] = useState<'search' | 'applied' | 'not_interested' | 'saved'>('search');
  const [selectedType, setSelectedType] = usePersistedState<JobType | 'any'>('tech-type', 'any');
  const [listedPeriod, setListedPeriod] = usePersistedState<ListedPeriod>('tech-period', 'any');
  const [jobStatus, setJobStatus] = usePersistedState<JobStatus>('tech-status', 'any');
  const [selectedCompanies, setSelectedCompanies] = usePersistedState<string[]>('tech-companies', []);
  const [selectedTitles, setSelectedTitles] = usePersistedState<string[]>('tech-titles', []);
  const [selectedSources, setSelectedSources] = usePersistedState<string[]>('tech-sources', []);
  const [filterKeywords, setFilterKeywords] = usePersistedState<string[]>('tech-keywords', []);
  const [sortBy, setSortBy] = usePersistedState<SortOption>('tech-sort', 'date-desc');
  const [datePostedFilter, setDatePostedFilter] = usePersistedState<DatePostedFilter>('tech-datePosted', 'all');
  ...
  const handleScrape = () => { startScrape('tech', sources, keywords, location); };
  const handleStopScrape = () => { stopScrape('tech'); };
  ...
  return (<>
    <div className="min-h-screen bg-background bg-grid">
      <NavBar activeTab="/tech" selectedCity={selectedCity} onCityChange={handleCityChange} isSearching={isSearching} onSearch={handleScrape} onStop={handleStopScrape} onSignInClick={() => setShowAuthModal(true)} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="lg:col-span-3 space-y-2">
            {viewMode === 'saved' ? (
              ...
              <div className="border border-border rounded-md bg-card p-12 text-center"><Zap className="h-8 w-8 text-primary mx-auto mb-3" /><p className="font-display text-sm text-foreground mb-1">Ready to scrape Tech jobs</p><p className="text-xs text-muted-foreground">Configure your sources, then click Find jobs</p></div>
