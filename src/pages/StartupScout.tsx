...
const StartupScout = () => {
  ...
  const [selectedPayRanges, setSelectedPayRanges] = usePersistedState<PayRange[]>('startups-payranges', []);
  const [customPayRange, setCustomPayRange] = usePersistedState<CustomPayRange>('startups-custompay', { min: null, max: null });
    ...
    <div className="min-h-screen bg-background bg-grid">
      ...
      <div className="border-b border-border bg-muted sticky top-[53px] z-40">
        <div className="container max-w-6xl mx-auto px-4 py-1.5 flex items-center gap-4 font-display text-[11px] uppercase tracking-wider text-muted-foreground">
          <span className="whitespace-nowrap">Startups</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="lg:col-span-3 space-y-2">
            {viewMode === 'saved' ? (
              ...
              <div className="border border-border rounded-md bg-card p-12 text-center"><Zap className="h-8 w-8 text-primary mx-auto mb-3" /><p className="font-display text-sm text-foreground mb-1">Ready to find Startup jobs</p><p className="text-xs text-muted-foreground">Configure your sources, then click Find Jobs</p></div>
