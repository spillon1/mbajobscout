...
/** Replace VC terms with MC in source URLs */
function toMCUrl(url: string): string {
  return url
...
const MCScout = () => {
  ...
  const [selectedCity, setSelectedCity] = usePersistedState<string>('mc-city', 'London');
  const location = getLocationString(selectedCity);
  const [sources, setSources] = useState<JobSource[]>(() => getMCDefaultSources('London'));
  const [keywords, setKeywords] = useState<string[]>(MC_DEFAULT_KEYWORDS);
  ...
  useEffect(() => {
    const result = consumeResults('mc');
    if (result) {
  ...
  const [selectedType, setSelectedType] = usePersistedState<JobType | 'any'>('mc-type', 'any');
  const [listedPeriod, setListedPeriod] = usePersistedState<ListedPeriod>('mc-period', 'any');
  const [jobStatus, setJobStatus] = usePersistedState<JobStatus>('mc-status', 'any');
  const [selectedCompanies, setSelectedCompanies] = usePersistedState<string[]>('mc-companies', []);
  const [selectedTitles, setSelectedTitles] = usePersistedState<string[]>('mc-titles', []);
  const [selectedSources, setSelectedSources] = usePersistedState<string[]>('mc-sources', []);
  const [filterKeywords, setFilterKeywords] = usePersistedState<string[]>('mc-keywords', []);
  ...
  const [customPayRange, setCustomPayRange] = usePersistedState<CustomPayRange>('mc-custompay', { min: null, max: null });
  ...
  const handleScrape = () => { startScrape('mc', sources, keywords, location); };
  const handleStopScrape = () => { stopScrape('mc'); };
    ...
    <div className="min-h-screen bg-background bg-grid">
      ...
      <div className="border-b border-border bg-muted sticky top-[53px] z-40">
        <div className="container max-w-6xl mx-auto px-4 py-1.5 flex items-center gap-4 font-display text-[11px] uppercase tracking-wider text-muted-foreground">
          <span className="whitespace-nowrap">Management Consulting</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <div className="lg:col-span-3 space-y-2">
            {viewMode === 'saved' ? (
              ...
              <div className="border border-border rounded-md bg-card p-12 text-center"><Zap className="h-8 w-8 text-primary mx-auto mb-3" /><p className="font-display text-sm text-foreground mb-1">Ready to find MC jobs</p><p className="text-xs text-muted-foreground">Configure your sources, then click Find Jobs</p></div>
