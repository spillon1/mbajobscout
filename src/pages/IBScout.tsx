...
import { getIBDefaultSources, IB_DEFAULT_KEYWORDS } from '@/data/ibData';
...
/** Replace VC terms with IB in source URLs */
function toIBUrl(url: string): string {
  return url
...
const IBScout = () => {
  ...
  const [selectedCity, setSelectedCity] = usePersistedState<string>('ib-city', 'London');
  const location = getLocationString(selectedCity);
  const [sources, setSources] = useState<JobSource[]>(() => getIBDefaultSources('London'));
  const [keywords, setKeywords] = useState<string[]>(IB_DEFAULT_KEYWORDS);
  ...
  useEffect(() => {
    const result = consumeResults('ib');
    if (result) {
      setJobs(result.jobs);
  ...
  const [selectedType, setSelectedType] = usePersistedState<JobType | 'any'>('ib-type', 'any');
  const [listedPeriod, setListedPeriod] = usePersistedState<ListedPeriod>('ib-period', 'any');
  const [jobStatus, setJobStatus] = usePersistedState<JobStatus>('ib-status', 'any');
  const [selectedCompanies, setSelectedCompanies] = usePersistedState<string[]>('ib-companies', []);
  const [selectedTitles, setSelectedTitles] = usePersistedState<string[]>('ib-titles', []);
  const [selectedSources, setSelectedSources] = usePersistedState<string[]>('ib-sources', []);
  const [filterKeywords, setFilterKeywords] = usePersistedState<string[]>('ib-keywords', []);
  ...
  const handleCityChange = (city: string) => {
    ...
    setSources((prev) =>
      prev.map((s) => ({
        ...s,
        url: toIBUrl(getSourceUrlForLocation(s.name, city) || s.url),
  ...
  const handleScrape = () => {
    startScrape('ib', sources, keywords, location);
  };
  ...
  const baseFilteredJobs = useMemo(() => {
    ...
    const NON_UK_INDICATORS = [
      ...
      /\b(united states|usa|\bUS\b|canada|australia|germany|france|india|singapore|hong kong|japan|china|brazil|israel|netherlands|switzerland|(?<!northern )ireland|spain|italy|sweden|denmark|norway|finland|austria|belgium|portugal|south korea|taiwan|thailand|vietnam|mexico|argentina|chile|south africa|nigeria|kenya|uae|dubai|saudi|qatar)\b/i,
    ];
    filtered = filtered.filter((j) => {
  ...
  return (<>
    <div className="min-h-screen bg-background bg-grid">
      <NavBar activeTab="/ib" selectedCity={selectedCity} onCityChange={handleCityChange} isSearching={isSearching} onSearch={handleScrape} onStop={handleStopScrape} onSignInClick={() => setShowAuthModal(true)} />
      ...
      <div className="border-b border-border bg-muted sticky top-[53px] z-40">
        <div className="container max-w-6xl mx-auto px-4 py-1.5 flex items-center gap-4 font-display text-[11px] uppercase tracking-wider text-muted-foreground">
          <span className="whitespace-nowrap">Investment Banking</span>
